...
func TestAvgFunc_GroupBy(t *testing.T) {
	g := NewGomegaWithT(t)
	...
	g.Expect(aggDs.Header().ColumnsMetadata()).To(HaveLen(2))
	g.Expect(rows).To(HaveLen(6))
	columns := []string{"AVG(population)", "city"}
	g.Expect(rows).To(ContainElements(
		HavingRowValues(columns, NewFloatValue(42615), NewStringValue("Dubrovnik")),
		HavingRowValues(columns, NewFloatValue(1848278.4), NewStringValue("London")),
		HavingRowValues(columns, NewFloatValue(1015826), NewStringValue("Odesa")),
		HavingRowValues(columns, NewFloatValue(12150.5), NewStringValue("Odessa")),
		HavingRowValues(columns, NewFloatValue(1527656.5), NewStringValue("Warsaw")),
		HavingRowValues(columns, NewFloatValue(42164.75), NewStringValue("Waterloo")),
	))
}
